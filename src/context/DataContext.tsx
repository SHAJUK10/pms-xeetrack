...
import { import { createContext, useContext, useState, useEffect, ReactNode, useCallback } from 'react';
import { createClient, SupabaseClient } from '@supabase/supabase-js';
import { Project, Stage, CommentTask, GlobalComment, File, Task, Meeting, BrochureProject, BrochurePage, PageComment, Lead, STAGE_NAMES, DownloadHistory, User } from '../types';
...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  const uploadFileFromInput = async (stageId: string, file: globalThis.File, uploaderName: string) => {
    ...
    try {
      ...
      console.log('Uploading file to storage:', storagePath, 'Bucket: project-files');
      ...
      const fileData: Omit<File, 'id' | 'timestamp'> = {
        ...
        project_id: projectId,
        filename: file.name,
        file_url: `${supabaseUrl}/storage/v1/object/public/project-files/${storagePath}`,
  ...
  const deleteFile = async (fileId: string, storagePath: string) => {
    ...
    try {
      ...
      console.log('Deleting file from storage:', storagePath, 'Bucket: project-files');
